...
knn_model = function(df_folds, n_test_fold, k){
  ...
  distances = as.matrix(daisy(rbind(df_folds[df_folds$fold != n_test_fold,-14],
                                     df_folds[df_folds$fold == n_test_fold,-14])))
...
performance_knn = function(predicted, y_test){
    ...
    results = c(sum(diag(cm)) / sum(cm), #Accuracy
                ...
                cm[2,1], #False Positives
                cm[1,2], #False negatives
...
```{r}
knn_validation(df_folds = df_folds, k = 13)
```
