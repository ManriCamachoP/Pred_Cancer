...
contrasts(data$BreastCancer) #As we can see we need to change so that No Breast Cancer is 0

data$BreastCancer = relevel(x = data$BreastCancer, ref = "No Breast Cancer")
```
...
na_data_long <- na_data_long %>%
  group_by(Variable) %>%
  mutate(Percentage = ifelse(Status == "Cantidad NA" & Count > 0, paste0(round((Count / sum(Count)) * 100, 1), "%"), ""))
